...
void IARManager::drawBackground()
{
    ...
    //draw2DImage 第二个参数决定AR的pic大小scrn_size.Width,scrn_size.Height
...
int IARManager::get_visible_id()
{
    int p,v_id=-1;
    for(p=0;p<this->patt_loaded;p++)
    {
        if(patt_node[p]->isVisible())
            v_id=p;
    }
    return v_id;
}
    ...
    if( pixels )
    {
        ...
        for(int i=0;i<max_pixels;i++)
        {
            ...
            *pixels = *ardata;
             ...
             *(pixels-2)= *pixels ^ *(pixels-2);
             *pixels =    *pixels ^ *(pixels-2);
             //the first and the third had been changed
